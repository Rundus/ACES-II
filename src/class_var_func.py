...
class newCDFvar:
    def __init__(self, zVar_str, data, modParam, model_data_object):
        ...
        if isinstance(data, list):
            self.var_data = array(data)
        elif data.dtype.type is str_: # exception: dataL1 is ndarray of strings
...
def tmCDF_TRICE(flier):
    return [Initialize_cdfFile(data_paths.TRICE_tmCDF_files[flier][i]) for i in range(len(
        data_paths.TRICE_tmCDF_files))]
