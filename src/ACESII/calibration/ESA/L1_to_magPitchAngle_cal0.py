...
def L1_to_L1ESAmagCal(wFile, wMagFile, rocketFolderPath, justPrintFileNames, wflyer):
    ...
    inputFiles = glob(f'{rocketFolderPath}{inputPath_modifier}\{stl.fliers[wflyer]}{modifier}\*.cdf')

    inputFiles_mag = glob(f'{rocketFolderPath}{inputPath_modifier_mag}\{stl.fliers[wflyer]}{modifier}\*RingCore*')

    outputFiles = glob(f'{rocketFolderPath}{outputPath_modifier}\{stl.fliers[wflyer]}\*.cdf')

    input_names = [ifile.replace(f'{rocketFolderPath}{inputPath_modifier}\{stl.fliers[wflyer]}{modifier}\\', '') for ifile in inputFiles]
    input_names_mag = [ifile.replace(f'{rocketFolderPath}{inputPath_modifier_mag}\{stl.fliers[wflyer]}\\', '') for ifile in inputFiles_mag]

    output_names = [ofile.replace(f'{rocketFolderPath}{outputPath_modifier}\{stl.fliers[wflyer]}\\', '') for ofile in outputFiles]
    ...
    dataFile_name = inputFiles[wFile].replace(f'{rocketFolderPath}{inputPath_modifier}\{stl.fliers[wflyer]}{modifier}\\', '')
    ...
    fileoutName = f'ACESII_{ACESII.payload_IDs[wRocket-4]}_l1_{wInstr[1]}_magPitch.cdf'
    ...
    if justPrintFileNames:
        ...
        # --- get the data from the l1 ESA file ---
        stl.prgMsg(f'Loading data from Mag Files')
        data_dict_mag = stl.loadDictFromFile(inputFiles_mag[wMagFile])
        ...
        if wInstr[0] in [0, 2]:  # EEPAA and # IEPAA
            ...
            unit_vect = [np.matmul(stl.Rx(45), temp_vect) for temp_vect in unit_vect_temp]
            ...
            unit_vect = [np.matmul(stl.Rx(180), temp_vect) for temp_vect in unit_vect_temp]
        ...
        stl.prgMsg('Creating Mag fullDataSet data\n')
        ...
        for tme, engy in tqdm(itertools.product(*[esaRanges[0], esaRanges[2]])):
            epochFullDataSet[tme][engy] = data_dict_esa['Epoch'][0][tme] + (esaLens[2] - 1 + energyAdjust - engy) * ACESII.timeBetweenSteps_in_ns
        ...
        outputPath = f'{rocketFolderPath}{outputPath_modifier}\{stl.fliers[wflyer]}\\{fileoutName}'
...
if len(glob(f'{rocketFolderPath}{inputPath_modifier}\{stl.fliers[wflyer]}\*.cdf')) == 0:
    ...
    if justPrintFileNames:
        ...
        for fileNo in (range(len(glob(f'{rocketFolderPath}{inputPath_modifier}\{stl.fliers[wflyer]}\*.cdf')))):
